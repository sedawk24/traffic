...
| 9 | Intersection capacity v2 + dark-cinematic viewer | Complete |
...
| B | Capacity v2: sweep harness + variant ladder (gateways, signal truthing, turn lanes, insertion, equilibrium w/ junction control) + `central_v2` showcase | Done | Ladder: V0 12.5 km/h @0.075 → V1 22.4 → V2 23.8 → V3/V4/V5 ~24; showcase config (equilibrium prior + rerouting + buses + coordination) runs scale 0.10 at 23.7 km/h → **run #41 central_v2** |
...
| S\* ≥ 0.075 flowing (mean ≥ 15 km/h, %stopped ≤ 45) | **Done — S\* = 0.10** | central_v2: 23.7 km/h, 34.5 % stopped, peak 4,926; teleports 5,307/2.5 h (not ≈0 — honest residual; #40's "0" was a low-density artifact) |
| Chokepoints fixed (diagnose before/after) | Done | #40: top-15 = 32 % of stopped time, 200–790 s/veh pathological queues → v2 (2× demand): top-10 = 12 %, real arterials at 18–66 s/veh; residual: Nelson St priority 507 s/veh (backlog) |
| GEH Granville/Cambie < 5 | Partial | Cambie 0.54 ✓; Granville 49 ✗ (False-Creek split shifted — per-crossing weights backlogged); Lions-Gate/viaduct screenlines are peninsula-only |
| More visible traffic at street zoom | Done | Same viewport/time: 1,770 → 4,253 on-road (2.4×); before/after in `docs/images/phase9_*` |
| Dark-cinematic viewer + extras render (smoke + screenshots) | Done | smoke green on run #41; interaction suite green; `docs/images/phase9_after_{street,flow,tilt}.png` |
...
| Date | Phase | Change |
|------|-------|--------|
| 2026-06-11 | 9 | **Phase 9 complete — run #41 `central_v2` is the new showcase.** Equilibrium at scale 0.125 (`sim equilibrium`, 7 meso iters w/ junction control, ~25 min) → percent samples; ceiling sweeps: frozen replay knees 0.075→0.10 (15 km/h, 57 % stopped), the **hybrid** (equilibrium prior + online rerouting + buses + 307-TLS green waves) holds **scale 0.10 at 23.7 km/h / 34.5 % stopped / peak 4,926** (p100 = 0.125 still 20.5). Showcase run #41: 40,065 veh, 38,948 completed, avg wait 253 s; same-viewport visible traffic 1,770 → 4,253 (2.4×). Diagnose before/after: #40's pathological 200–790 s/veh queues (top-15 = 32 % of stopped time) → v2 top-10 = 12 % at 18–66 s/veh on the city's real busiest signals; residual Nelson-St priority outlier backlogged. Calibration honest: Cambie GEH 0.54 ✓, Granville split shifted (GEH 49, backlogged per-crossing weights; calibrate's Lions-Gate/viaduct screenlines are peninsula-only). API now remaps registry trace paths onto the local `data/` so a copied repo works (414 MB laptop bundle + `LAPTOP.md` shipped). After-screenshots in `docs/images/`. |
| 2026-06-11 | 9 | **Phase 9B — the capacity ceiling falls (measured ladder).** New `sim sweep` harness (fixed-seed scale ladder; FCD-weighted mean km/h, %stopped, peak, completions, teleports from the new stats output; `--no-tls-add` for raw-net A/B fairness). Ladder vs the V0 baseline (one-shot routing, raw signals, scale 0.075 = **12.5 km/h, 58.8 % stopped**): **V1 demand gateways** — external CSDs now enter on the net's severed-arterial stubs (Kingsway/Hastings/Broadway/Terminal E, bridge approaches S, Causeway N, 4th/10th/16th W), capacity/distance²-weighted, instead of "80 nearest side-street edges" → **22.4 km/h (+79 %), completions +74 %**; **V2 signal ground-truthing** — new `etl signal-truth --area` reconciles the net against the CoV inventory via a plain-XML round-trip (`--tls.unset` can't strip baked-in TLS), 700→466 signalized nodes → **23.8 km/h (+6 %)**; **V3 turn-lane import + street names** (`--osm.turn-lanes`, `--output.street-names` now standard; rebuild quarantines stale tls add-files) → marginal alone (+0–3 %, teleports −8 %), kept for realism at zero cost; **V4/V5** insertion attrs (`departLane=best departSpeed=max departPos=random_free`) + home trip-ends off primary mid-blocks. `sim equilibrium` wraps duaIterate reproducibly (pre-cleans unroutable trips with `duarouter --write-trips`; duaIterate owns expand/ignore-errors/teleport/no-warnings options — don't duplicate; **`--meso-junction-control`** so junction friction prices routes, fixing the meso-blind equilibrium that jammed on micro replay); `sim retime` wraps tlsCoordinator/tlsCycleAdaptation. |
| 2026-06-11 | 9 | **Phase 9C/D — dark-cinematic viewer shipped.** `web/` rebuilt as ES modules (no build step): runtime-darkened basemap (openfreemap `dark` or recolored positron), glow flow ribbons (additive 2-pass), sprite atlas v2 with meter-true sizes + per-vehicle hue jitter, trails on by default (old `show.trails` was dead code), signal halo+core heads, LOD icons from z12.6, HUD (clock/day-part top bar, Live/Worst-intersections/Layers/Legend rail, timeline scrubber with congestion-tinted histogram), street-name tooltips, keyboard shortcuts, 3D building extrusions + tilt, click-to-follow chase cam, hotspot panel (flyTo + pulse) fed by `sim diagnose`. API: names in network GeoJSON, `vid` in `/trips`, `/timeline`, `/hotspots`, `/api/signals?area=`. Perf rules learned by profiling real hangs: stable deck `data` references (a per-frame concat re-uploaded ~600 k trail vertices/frame) and **no infinite CSS animations** (one animated 9-px dot forced full-page WebGL recomposites). Verified: smoke script + interaction suite + before/after screenshots (`scripts/screenshot.py`, `scripts/smoke_viewer.py`, playwright dev dep). |
