...
- **Windowed/streaming trace replay (unlocks real density).** The viewer loads *all* trips upfront, so it can't replay real-density micro runs (calibration shows real ≈ scale 0.55 = ~170k vehicles / ~1 GB trace). Stream a time-window of the trace (the `/api/runs/{id}/trace` Arrow endpoint already serves windows) as the scrubber moves, so a full-real-density city/district run is viewable. This is the single biggest unlock for "real traffic levels" on screen.
