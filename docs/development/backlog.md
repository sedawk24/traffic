...
- ~~**Windowed/streaming trace replay (unlocks real density).**~~ **DONE (2026-06-02):** `/api/trips` takes `start`/`end` (predicate-pushed at the parquet level); the viewer streams a 300 s window (`every=3`) for dense runs (>8k veh) and reloads as the clock advances. A 34k-vehicle run that produced a 685 MB trips payload (browser couldn't load it) now streams ~12 MB windows. *Follow-ups:* sort the trajectory parquet by `t` so window reads are <1 s (currently ~2-4 s on the unsorted file); ~~prefetch the next window during playback~~ **DONE (2026-06-11, Phase 9C — windows prefetch ahead of the clock, `every=2`).**
- **Per-bridge demand balance (central).** `central_v2` (Phase 9) calibration: Cambie GEH 0.54 ✓ but the False-Creek split shifted — Granville bridge under-used (2,661 sim vs 5,850 obs, GEH 49) and Burrard slightly over (GEH 10.7). The Phase-9 demand restructure (gateway stubs + home-end pools) moved interior crossing choices; add per-crossing demand weights (like the peninsula `GATEWAY_WEIGHT`) or duaIterate-level corrections and re-verify all three to GEH<5. Note: the calibrate tool's Lions Gate/viaduct screenlines are peninsula-cordon lines that don't apply to the central bbox (Lions Gate midspan is outside the net) — `sim calibrate` should grow an `--area` notion before its global scale fit is trusted on central runs.
- **Residual jam outliers (central_v2).** `sim diagnose` on the new showcase: top-10 junctions are now real signalized arterials at normal 18–66 s/veh stopped, but one pathological priority junction survives (Nelson St, 507 s/veh, 29.7 veh·h) plus a Main St mid-block priority node (#3) — candidates for a small netdiff fix or signal check.
...
- ~~**Better basemap styling**~~ **DONE (2026-06-11, Phase 9C):** dark-cinematic runtime restyle of the hosted positron style (openfreemap `dark` preferred when available). A bespoke hosted style remains an option with the PMTiles item below.
- **Self-hosted PMTiles basemap (offline / no-cloud).** Generate a Protomaps PMTiles BC extract (needs tilemaker/planetiler or the `pmtiles` CLI — none installed) + a Protomaps MapLibre style, served via range requests. OpenFreeMap (runtime-darkened) is the dev basemap until then (Phase 3 Task 1, deferred).
- **Viewer refinements.** Phase 9C/D landed: sprite atlas v2 (car/bus/truck/van), trails by default, signal halos, timeline histogram, tooltips, follow mode, hotspot panel, 3D buildings + tilt. Remaining: SkyTrain icons once demand includes rail; sprite art beyond silhouettes (see the SimCity skin item); a `delivery` class in the trace (vans currently render as trucks).
- ~~**Raising the flowing ceiling (a major project, not a tweak).**~~ **LARGELY DONE (2026-06-11, Phase 9B — measured):** the ceiling was *not* fundamental — it was (1) the demand model funneling all eastern-region OD into Chinatown side streets (fixed: severed-arterial gateway pools, capacity/d²-weighted), (2) ~240 ground-truth-bogus signals (fixed: `etl signal-truth` vs the CoV inventory), (3) discarded OSM `turn:lanes` and speed-0/lane-0 insertions (fixed). Raw one-shot sweep at scale 0.075: **12.5 → ~24 km/h**; scale 0.10 flows at ~20 km/h (was 6–12). Residual: priority-junction friction for minor-cross-arterial moves under heavy flow; per-junction netdiff fixes for any survivors in the new top-10.
...
- **Whole-city route equilibrium.** Run `duaIterate` on the full `vancouver` 76k-edge net — now wrapped as `sim equilibrium` (Phase 9B adds `--meso-junction-control` so junction friction prices the routes); whole-city remains a multi-hour job.
- **Port Phase 9 capacity fixes to the other areas.** `vancouver` and `peninsula` nets predate Phase 9: rebuild them to pick up `--osm.turn-lanes` + `--output.street-names`, run `etl signals --area` + `etl signal-truth --area` (CoV data covers the whole city; the peninsula would also drop its bogus TLS), and they inherit the gateway/insertion demand fixes automatically. Metro (meso, arterials-only, outside CoV coverage) only gets the netconvert flags.
