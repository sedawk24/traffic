...
- **Network-capacity tuning (raise the flowing ceiling toward real demand).** The auto-generated net's effective capacity is ~half of real — one-shot routing gridlocks around scale ~0.25 while calibrated real demand is ~0.55. Done so far: duaIterate equilibrium (busy-and-moving at 0.15), arterial routing bias (keeps traffic off side streets), and `tlsCoordinator` green-wave **signal coordination** (−16 % wait / +9 % throughput, 8c). Remaining levers for *true* demand: **lane-count corrections** (OSM under-tags arterial lanes) and **turn-lane/connection cleanup** to add real arterial capacity. Measured negative result still stands — per-intersection cycle adaptation (`tlsCycleAdaptation`) did *not* clear spillback, so capacity (lanes), not cycle length, is the remaining lever.
