...
- **Phase 9 finale:** `sim calibrate`/`sim diagnose` snapshot of `central_v2` + after-screenshots (the run itself is registered and viewable).
- **Port Phase 9 fixes to `vancouver`/`peninsula`** (rebuild with new flags + signal-truth + the inherited demand fixes), then the whole-city equilibrium (multi-hour) — see backlog.
