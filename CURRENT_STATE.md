...
**Status: Phase 9 — the Phase-8c "capacity ceiling" has fallen, and the viewer is rebuilt dark-cinematic. The ceiling wasn't fundamental: `sim diagnose` traced the mega-jams to the demand model funneling every eastern suburb into Chinatown side streets, ~240 ground-truth-bogus traffic lights (700 net TLS nodes vs ~298 real CoV vehicle signals), discarded OSM turn lanes, and stopped-dead insertions. Fixed and measured (fixed-seed `sim sweep`): at scale 0.075 the net went 12.5 → ~24 km/h; the new showcase config — user-equilibrium routes (junction-control-priced) + online rerouting + buses + green-wave coordination — runs scale 0.10 (DOUBLE run #40's demand) at 23.7 km/h mean, 34.5 % stopped, ~4,900 peak on-road, 40 k vehicles over 07:00–09:30 (`central_v2`). The viewer is now a modular dark-cinematic app: glow flow ribbons, sprite vehicles with trails, halo signal heads, live-stats HUD, timeline histogram scrubber, street-name tooltips, 3D buildings + tilt, click-to-follow, and a worst-intersections panel driven by the new diagnostics. Phases 0–6 complete; 7 (metro meso) + 8 (full city) in progress; Phase 9 capacity+UI overhaul essentially complete (final calibration/diagnose snapshot of central_v2 pending).**
...
## Phase 9 — Intersection capacity v2 + dark-cinematic viewer (essentially complete)

Driven by three user complaints: too little visible traffic at street zoom, terrible jams at specific intersections, and a lackluster viewer. Plan: `docs/development/phases/phase-9.md`.

**A — Diagnostics.** New `sim diagnose --run <name>` ranks junctions by stopped vehicle-hours from a run's FCD, names them from the OSM extract / net street names, and ground-truths each against the **real CoV signal inventory** (`etl signals --area` — new `kind` column; 481 signals in the central bbox = 298 vehicle + 180 ped-only, vs **700 TLS junction nodes in the net**). librun now emits `edgedata.xml`/`stats.xml`/`summary.xml`; `/api/runs/{id}/hotspots` serves the ranking. Run #40's diagnosis: top-15 junctions = 32 % of all stopped time, dominated by one standing queue along Main/Hastings/Powell/Gore — the demand model funneled ALL eastern-region OD into each CSD's 80 *nearest* (NE-corner side-street) edges.

**B — Capacity v2 (measured ladder, fixed-seed `sim sweep`, scale 0.075 raw baseline 12.5 km/h / 58.8 % stopped):** V1 severed-arterial **gateway pools** (capacity/d²-weighted per suburb) → 22.4 km/h (+79 %), completions +74 %; V2 **signal ground-truthing** (`etl signal-truth`, plain-XML round-trip, 700→466 signalized nodes) → 23.8 km/h; V3 **turn-lane import + street names** (now standard netconvert flags) → marginal alone, kept for realism; V4/V5 insertion attrs (`departLane=best departSpeed=max departPos=random_free`) + home trip-ends off primary mid-blocks. **`sim equilibrium`** wraps duaIterate reproducibly (pre-cleans unroutable trips; **`--meso-junction-control`** prices junction friction — the 8c equilibrium was meso-blind and jammed on micro replay); **`sim retime`** regenerates tlsCoordinator green waves (307 TLS). **Ceiling result:** frozen-equilibrium replay knees between 0.075 (22.9 km/h) and 0.10 (15.0); the showcase config (equilibrium prior + online rerouting + buses + coordination) runs **scale 0.10 at 23.7 km/h, 34.5 % stopped, peak ~4,900 on-road, 40 k vehicles** (p100 = scale 0.125 still flows at 20.5).

**The deliverable — run #41 `central_v2`** (scale 0.10, 07:00–09:30, buses + coordination + equilibrium-prior routing): **40,065 vehicles, 4,926 peak on-road, 23.7 km/h mean, 38,948 trips completed, avg wait 253 s** — vs run #40's scale 0.05 / 2,370 peak / 16.6 km/h. At the same street viewport the viewer shows **4,253 on-road where #40 showed 1,770 (2.4×)**. **Chokepoints (diagnose before/after):** #40's top-15 held 32 % of stopped time with pathological 200–790 s/veh standing queues (Chinatown funnel); v2's top-10 hold **12 %** and are the city's real busiest signals (Terminal & Main, 12th & Main, Broadway & Main, 33rd & Granville/Oak) at a **normal 18–66 s stopped/vehicle**. Known residuals (backlog): one pathological priority junction (Nelson St, 507 s/veh); **calibration**: Cambie GEH 0.54 ✓ but the False-Creek split shifted (Granville bridge under-used, GEH 49; Burrard 10.7) — per-crossing demand weights are the named follow-up, and `sim calibrate`'s Lions-Gate/viaduct screenlines are peninsula lines that don't apply to the central bbox.

**C/D — Viewer.** Rebuilt as ES modules (no build step): runtime-darkened basemap, glow flow ribbons, sprite atlas v2 (meter-true, hue-jittered), trails on by default, halo signal heads, LOD from z12.6, HUD (clock/day-part, live stats, worst-intersections panel, layers, legend, timeline histogram scrubber), street-name tooltips, keyboard shortcuts, 3D buildings + tilt, click-to-follow chase cam, hotspot flyTo + pulse. Verified headlessly (`scripts/smoke_viewer.py`, `scripts/screenshot.py`, interaction suite) — screenshots in `docs/images/`.

## What Is Next (Phase 9 is complete — pickup points)

1. **False-Creek bridge split** (top backlog item): per-crossing demand weights so Granville bridge regains its share (GEH 49 → <5); teach `sim calibrate` an `--area` notion (its Lions-Gate/viaduct screenlines are peninsula-only).
2. **Residual jam outliers:** Nelson St priority junction (507 s/veh) + Main St mid-block node — small netdiff or signal-truth threshold check.
3. **Port Phase 9 to `vancouver`/`peninsula`:** rebuild nets (new flags are already standard), `etl signals --area` + `etl signal-truth --area`, then the whole-city equilibrium (multi-hour) — see backlog.
4. Housekeeping: `data/` grew to ~25 GB (sweeps + dua intermediates are safe to delete: `data/runs/sweeps/`, `data/runs/*_dua/iter*`, `*.pretruth`/`*.bak`/`*.stale`); a 414 MB laptop bundle recipe is in the 2026-06-11 session (LAPTOP.md documents how a copied repo runs).
...
*Last updated: 2026-06-11 (Phase 9 — capacity ceiling broken: scale 0.10 flowing at 23.7 km/h; dark-cinematic viewer shipped)*
